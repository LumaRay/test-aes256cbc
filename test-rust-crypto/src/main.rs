...
// Encrypt
    let mut encrypted_data = encrypt(&message, &key, &iv).ok().unwrap();
    let start = SystemTime::now();
    for _ in 0..LOOPS_COUNT {
        encrypted_data = encrypt(&message, &key, &iv).ok().unwrap();
    }
    println!("Encrypt {:?}", SystemTime::now().duration_since(start).unwrap());

// Decrypt
    let mut decrypted_data = decrypt(&encrypted_data[..], &key, &iv).ok().unwrap();
    assert!(message == &decrypted_data[..]);
    let start = SystemTime::now();
    for _ in 0..LOOPS_COUNT {
        decrypted_data = decrypt(&encrypted_data[..], &key, &iv).ok().unwrap();
    }
